...
Voice-first AI companion. Runs fully local by default, with optional cloud-LLM fallback. This document captures the architecture, flow, and the decisions that would otherwise live only in my head.

Architechture overview

A single orchestrator wires five stages together. Each stage is swappable module behind a small interface, so a component can be moved or replaced without touching the rest of the pipeline.
_______________________________________________________________________________________
|                                                                                      |
|    Desktop (9600x, 32gb - my rig)                                                    |
|                                                                                      |
...
loop:
    wait_for_wakeword()             # SLEEP - Porcupine blocks until 'Hey Somi"
    ...
    speech = tts(response)          # THINK - Qwen3-TTS OR remote (toggle)
    play(speech)                    # SPEAK - sounddevice playback

Interruption ("barge-in") - the user starts talking while Somi is speaking - is a later concern. v.01 is a blocking loop; asyncio comes when interruption matters.
...
LLM         llama-server (HTTP) LLM Box             GPU that fits LLM of choice
...
One interface, two backends, mirroring LLM pattern.
...
- local -> model loaded in process, CPU inference on desktop
- remote -> SOMI_TTS_URL=http://<llm-box>:8080/v1/audio/speech (small server, built later; the remote client is a stub from day one)
...
Secrets stay out of git - .env is already ignored. Load with python-dotenv or manual os.getenv.

Decision Log
- Porcupine for wake word - local, low latency, small footprint, free-tier key. Custom wake word ('Hey Somi') via a .ppn file; built-in fallback for testing.
- faster-whisper for STT - CTranslate2 backend is much faster on CPU than stock Whisper; no GPU required, so it stays on the desktop.
- Qwen3.6-27B (Q4_K_M) for the LLM - the only machine that fits it is my personal LLM Box. Serverd over the LAN by llama-server (systemd) rather than loaded into the desktop process.
- OpenAI-compatible HTTP everywhere - llama-server exposes /v1/chat/completions, which means the LLM client is identical whether the brain is local or cloud API. This is what makes the local/api toggle trivial.
- Qwen3-TTS for TTS (over Piper/Kokoro) - chosen voice model. Runs on desktop CPU by default; TTS is latency-sensitive and lives on the machine with the speaker. The LLM Box is a CPU offload option for edge cases, not the default.
- Florence-2 for screen awareness - planned so Somi can see/describe what's on screen. Local to the desktop (it needs the screen). Not started.
